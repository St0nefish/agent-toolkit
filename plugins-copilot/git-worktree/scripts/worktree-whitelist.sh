...
is_git_worktree_cmd() {
  local cmd="$1"
  # Match: git [global-flags] worktree <subcmd>. Bash regex for BSD/GNU portability
  # (macOS grep lacks -P/PCRE).
  [[ "$cmd" =~ ^[[:space:]]*git[[:space:]] ]] || {
    WT_SUBCMD=""
    return 1
  }
  if [[ "$cmd" =~ [[:space:]]worktree[[:space:]]+([^[:space:]]+) ]]; then
    WT_SUBCMD="${BASH_REMATCH[1]}"
    return 0
  fi
  WT_SUBCMD=""
  return 1
