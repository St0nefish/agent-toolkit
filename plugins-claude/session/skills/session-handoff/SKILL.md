...
5. Confirm to the user:
   ...
   - How to resume: `git pull && /session:session-resume` on the other machine

### Notes

- The WIP commit is a normal commit — continue working on top of it, no soft-reset needed
- If no issue is linked, context lives only in the WIP commit; instruct the user to run `/session:session-resume` on the other machine
