...
This command invokes the `ship` skill — see `${CLAUDE_PLUGIN_ROOT}/skills/ship/SKILL.md` for the full procedure. The summary:
...
Use the `git-cli` wrapper at `${CLAUDE_PLUGIN_ROOT}/scripts/git-cli` for every GitHub/Gitea call. Never invoke raw `gh` or `tea` directly.
