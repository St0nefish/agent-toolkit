...
```bash
/permission-manager:permission-config commands add --scope project
...
The `/permission-manager:permission-config learn` action analyzes your audit log to find commands that are frequently prompted and suggests glob patterns:

```bash
/permission-manager:permission-config learn
...
```bash
/permission-manager:permission-config allow-edit
```
...
```bash
/permission-manager:permission-config web
```
...
```bash
/permission-manager:permission-config explain 'git push origin feature/42-export && docker exec app cat /etc/nginx.conf'
```
